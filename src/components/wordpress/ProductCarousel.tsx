...
import { ChevronLeft, ChevronRight, ArrowRight, ShoppingCart, Tag } from 'lucide-react';
...
const ProductCard = ({ product, isActive, onHover }) => {
  const imageUrl = getFeaturedImageUrl(product, 'medium') || 'https://via.placeholder.com/300x200';
  const title = product.title.rendered || 'Untitled Product';
  const excerpt = product.content?.rendered?.substring(0, 120) + '...' || '';
